...
public class Playlist implements Serializable{
	 ...
	 */
	public void addTrack(Song track, Album album) {
		PlaylistEntry playlistEntry = new PlaylistEntry();
//		playlistEntry.setAlbum(album);
		playlistEntry.setSong(track);
	 ...
	 * @param album <code>Album</code> instance
	 */
	public void addTracks(Song[] tracks, Album album) {
		for(int i=0; i<tracks.length; i++){
			addTrack(tracks[i], album);
		}
	}
	
	public void addTracks(List<Song> tracks) {
	...
	public void selectOrAdd(Song track, Album album){
		
		// first search thru available tracks
		for(int i=0; i<playlist.size(); i++){
			if(playlist.get(i).getSong().getObjectId() == track.getObjectId()){
				select(i);
				return;
			}
		}
