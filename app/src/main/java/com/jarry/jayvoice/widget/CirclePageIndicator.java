...
public class CirclePageIndicator extends View implements PageIndicator {
	...
	protected void onDraw(Canvas canvas) {
		...
		float cx = (mSnap ? mSnapPage%count : mCurrentPage % count) * threeRadius;
		if (!mSnap) {
	...
	public void onPageSelected(int position) {
		if (mSnap || mScrollState == ViewPager.SCROLL_STATE_IDLE) {
			...
			if(this.myPageChangeListener!=null){
				this.myPageChangeListener.pageChange(position);
			}
		}
	...
	int getRealCount() {
		if (mViewPager.getAdapter() instanceof LoopPagerAdapter) {
			LoopPagerAdapter adapter = (LoopPagerAdapter) mViewPager.getAdapter();
